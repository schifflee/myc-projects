...
namespace USBWatcher
{
    public partial class USBWatcher : OfficeForm
    {
        ...
        //private List<Partition> GetPartitionList(UDISK udisk)
        //{
        //    List<Partition> partitionlist = new List<Partition>();
        //    foreach (UDISK disk in udisks)
        //    {
        //        if (disk.DiskIndex==udisk.DiskIndex)
        //        {
        ...
        //                {
        //                    Index = UDISKUtil.GetPartitionIndex(letter),
        //                    Letter = letter,
        //                    Volume = UDISKUtil.GetVolumeLabel(letter),
        //                    Format = UDISKUtil.GetPartitionFormat(letter),
        //                    TotalSpace = UDISKUtil.GetPartitionTotalSpace(letter),
        //                    FreeSpace=UDISKUtil.GetPartitionFreeSpace(letter)
        //                };
        ...
        //}
        private List<UDISK> GetUDISKList()
        {
            List<UDISK> udisklist = new List<UDISK>();
            UDISK udisk = null;
            foreach (int index in udiskindexes)
            {
                udisk = new UDISK
                {
                    DiskIndex = index,
                    Caption = UDISKUtil.GetDISKCaption(index),
                    DiskType = UDISKUtil.GetDISKType(index),
                    Letters = UDISKUtil.GetLetters(index),
                    TotalSpace = UDISKUtil.GetTotalSpace(index)
                };
                udisklist.Add(udisk);
            }
            return udisklist;
        }
        private List<Partition> GetPartitionList(int index)
        {
            List<Partition> partitionlist = new List<Partition>();
            Partition partition;
            foreach (UDISK udisk in GetUDISKList())
            {
                if (udisk.DiskIndex == index)
                {
                    udisk.Letters = UDISKUtil.GetLetters(index);
                    foreach (string letter in udisk.Letters)
                    {
                        partition = new Partition
                        {
                            Letter = letter,
                            Index = UDISKUtil.GetPartitionIndex(letter),
                            Volume = UDISKUtil.GetVolume(letter),
                            Format = UDISKUtil.GetPartitionFormat(letter),
                            TotalSpace = UDISKUtil.GetPartitionTotalSpace(letter),
                            FreeSpace = UDISKUtil.GetPartitionFreeSpace(letter),
                        };
        ...
        {
            this.panel1.Visible = true;
            udiskindexes = UDISKUtil.GetDISKIndexes();
        ...
        private UDISK GetUDISK(int udiskindex)
        {
            UDISK udisk = null;
            ...
            foreach (int index in udiskindexes)
            {
                if (index == udiskindex)
                {
                    partitionlist=GetPartitionList(index);
                    udisk = new UDISK(index, partitionlist);
                }
            }
            return udisk;
        }
